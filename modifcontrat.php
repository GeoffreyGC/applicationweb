...
if(isset($_POST['modif'])){
    $sql2 = "UPDATE contrat SET contrat = ? WHERE idcontrat = ?";
    
    $rs_modif = $pdo->prepare($sql2);
    $var_contrat = $_POST['contrat'];
    $var_contratID = $_POST['ID'];
    
    $rs_modif->bindValue(1,$var_contrat,PDO::PARAM_STR);
    $rs_modif->bindValue(2,$var_contratID,PDO::PARAM_STR);
 
    if($rs_modif->execute()){
    
    $message = '<p class="msgGreen">Type de contrat modifiée</p>';
    } else {
...
$var_Idcontrat = $_GET['id'];
$rs_select->bindValue(1,$var_Idcontrat,PDO::PARAM_INT);
$rs_select->execute();
...
<html>
    <head>
        <title>Modifier un type de contrat</title>
...
crossorigin="anonymous"/>
   ...
   <form action="" method="post" name="formulaire" class="col-6 bg-light">
            ...
            <div>
                ...
                <input type="text" id="contrat" name='contrat' value="<?= $contrat ;?>" size="32" required>
            </div>

            <br/>
            
            <input type="hidden" name="ID" value="<?= $_GET['id'] ?>">
            <input type="submit" name="modif"> <input type="reset" value="effacer" name="new">
            </br>
            </br>
            <a class="leBouton" href="listecontrat.php">Retour</a>
            <br/>
            <p>   </p>
            </fieldset>
        </form>
