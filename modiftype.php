...
if(isset($_POST['modif'])){
    $sql2 = "UPDATE typeinter SET typeintervention = ? WHERE idtype = ?";
    
    $rs_modif = $pdo->prepare($sql2);
    $var_type = $_POST['type'];
    $var_typeID = $_POST['ID'];
    
    $rs_modif->bindValue(1,$var_type,PDO::PARAM_STR);
    $rs_modif->bindValue(2,$var_typeID,PDO::PARAM_STR);
 
    if($rs_modif->execute()){
    
    $message = "<p class='msgGreen'>Type d'intervention modifiée</p>";
    } else {
...
if(isset($_GET['id'])) {
$sql = "SELECT * FROM typeinter WHERE idtype = ?";
$rs_select = $pdo->prepare($sql);

$var_Idtype = $_GET['id'];
$rs_select->bindValue(1,$var_Idtype,PDO::PARAM_INT);
$rs_select->execute();
...
crossorigin="anonymous"/>
   ...
   <form action="" method="post" name="formulaire" class="col-6 bg-light">
            ...
            <div>
                ...
                <input type="text" id="type" name='type' value="<?= $type ; ?>" size="32" required>
            </div>
            ...
            <a class="leBouton" href="listetype.php">Retour</a>
            <br/>
            <p>   </p>
            </fieldset>
        </form>
