...
$var_Type = $_POST['type'];
        ...
        $rs_insert = $pdo->prepare("INSERT INTO typeinter (typeintervention) VALUES (?)");
   
        $rs_insert->bindValue(1,$var_Type,PDO::PARAM_STR);

        if($rs_insert->execute()){
        
        $message = '<p class="msgGreen">Type intervention enregistrée</p>';
        
        
        }else{
            $message = "<p class='msgRed'>Probleme avec l'enregistrement du type d'intervention, veuillez ressayer</p>"; 
        }
    ...
    <body>
        ...
        <form action="" method="post" name="formulaire" class="col-12 text-center bg-light">
            ...
            <div>
                ...
                <input type="text" id="type" name='type' placeholder="Veuillez entrer le type d'intervention" size="32" required>
            </div>
            ...
            <a class="leBouton" href="addtype.php">Nouveau type d'intervention</a>
            <br/>
            <p>   </p>
            </fieldset>
        </form>
